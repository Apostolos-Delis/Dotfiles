---
description: Analyze the project, suggest practical hooks, and create them with proper testing
...
- PostToolUse hook: "Auto-format with black/isort after editing"
- PostToolUse hook: "Run ruff/flake8 lint checks"
- PreToolUse hook: "Block commits with mypy type errors"

**When Ruby tooling is detected (`Gemfile`, `.rubocop.yml`):**

- PostToolUse hook: "Auto-format with rubocop -a after editing"
- PreToolUse hook: "Block commits with rubocop offenses"
...
Project has Python? -> Suggest black/ruff/mypy hooks
Project has Ruby? -> Suggest rubocop hooks
...
3. **Verify expected behavior**: Check if it blocks/warns/provides context as intended
...
### Python Linting (PostToolUse)

```bash
#!/bin/bash
# Read stdin JSON, check .py files only
# Run: ruff check --fix [file] && black [file]
# Output: JSON with suppressOutput: true on success
```
