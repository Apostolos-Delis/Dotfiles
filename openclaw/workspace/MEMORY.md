...
- The desired workflow is: chat with the orchestrator, create or attach coding-agent sessions, monitor them in parallel, follow up when they stall, verify the work, and prepare or create PRs.
...
- The bundled `coding-agent` skill should be enabled and used for background workers. Default to Codex, but use Claude Code or OpenCode when the user specifies that harness. The user should not need to manually create Workboard cards for normal fleet orchestration.
- For worker lanes, prefer the push-based flow: launch background workers, monitor with `process`/tasks/session tools, and notify only on blocked, failed, or PR-ready states.
